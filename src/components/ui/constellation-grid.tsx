...
export default function ConstellationGrid({
  ...
  useEffect(() => {
    ...
    const render = (now: number) => {
      ...
      const bgColor = isDarkMode ? "#07080d" : "#f8fafc";
      ...
      for (let i = 0; i < nodes.length; i++) {
        ...
        for (let j = i + 1; j < nodes.length; j++) {
          ...
          if (distSq < MAX_CONN_DIST_SQ) {
            ...
            const alpha =
              (1 - nDist / MAX_CONN_DIST) * (isDarkMode ? 0.18 : 0.08);
  ...
  return (
    <div
      ref={containerRef}
      className={cn(
        "relative w-full overflow-hidden select-none bg-[#07080d]",
        className,
      )}
    >
      <canvas
        ref={canvasRef}
        className="absolute inset-0 block cursor-crosshair z-0 pointer-events-none"
      />
